...
use rustty::{Terminal, CellAccessor, HasSize, Color};

use terrain::TerrainMap;
use hexpos::{OffsetPos, Pos};

pub fn draw_overhead_map(term: &mut Terminal, map: &TerrainMap, selected_pos: Option<Pos>) {
    let _ = term.clear();
    ...
    for ih in 0..maph {
        for iw in 0..mapw {
            ...
            if let Some(cell) = term.get_mut(iw as usize, ih as usize) {
